...
var secretKey = []byte(os.Getenv("JWT_SECRET_KEY")) // Загрузка ключа из переменной окружения
...
func CreateToken(email string, roles []string) (string, error) {
	...
	log.Printf("Секретный ключ: %s", secretKey) // Для тестов, потом удалите
	...
	log.Printf("Успешно создан токен: %s", tokenString)
